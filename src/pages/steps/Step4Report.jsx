...
function parseMd(text) {
  ...
  return text
    ...
    .replace(
      /```[\s\S]*?```/g,
      (m) =>
        `<pre class="bg-s2 border border-white/[0.09] rounded-md p-4 text-xs font-mono overflow-x-auto my-3 leading-7 text-emerald-300 whitespace-pre-wrap">${m.slice(3, -3).replace(/^[a-z]*\n/, "")}</pre>`,
    )
    .replace(
      /`([^`]+)`/g,
      '<code class="font-mono text-[11.5px] bg-s3 px-1.5 py-0.5 rounded text-emerald-300">$1</code>',
    )
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-[11px] font-semibold text-t3 uppercase tracking-[1.2px] mt-6 mb-2 pb-1.5 border-b border-white/5 first:mt-0">$1</h2>',
    )
    .replace(
      /\*\*(.+?)\*\*/g,
      '<strong class="text-t1 font-medium">$1</strong>',
    )
    .replace(/^- (.+)$/gm, '<li class="mb-1 text-t2">$1</li>')
    .replace(
      /(<li[\s\S]*?<\/li>\n?)+/g,
      (m) => `<ul class="pl-5 mb-3">${m}</ul>`,
    )
    .replace(/\n\n+/g, '</p><p class="mb-3 text-t2">')
    .replace(/\n/g, "<br>");
}
    ...
    <div className="animate-fade-up">
      ...
      <div className="bg-[var(--gd)] border border-accent/20 rounded-lg px-6 py-5 flex items-center gap-3.5 mb-6 animate-fade-up">
        <div className="w-9 h-9 bg-accent rounded-full flex items-center justify-center flex-shrink-0">
          <svg
            className="w-[18px] h-[18px] stroke-white fill-none stroke-[2.5] [stroke-linecap:round] [stroke-linejoin:round]"
            viewBox="0 0 24 24"
          >
        ...
        <div>
          <strong className="block text-[14px] font-medium text-t1 mb-0.5">
            리포트 생성 완료
          </strong>
        ...
        <div className="flex-1 flex flex-col gap-2.5">
          {(radar || []).map((d) => (
            <div key={d.label} className="flex items-center gap-2.5">
              <span className="text-[12px] text-t2 w-[72px] flex-shrink-0">
                {d.label}
              </span>
              ...
              <span className="text-[11px] text-t3 font-mono w-9 text-right">
                {d.val}점
              </span>
            </div>
          ))}
      ...
      <div className="bg-s1 border border-white/5 rounded-lg p-6">
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: '<p class="mb-3 text-t2">' + parseMd(report) + "</p>",
          }}
        />
      </div>
