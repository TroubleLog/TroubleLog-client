...
import { MOCK_CODE, MOCK_QUESTIONS, MOCK_REPORT, MOCK_RADAR, MOCK_FEEDBACK } from './data/mockData'
import { toISO, loadHist, saveHist, seedHist, getNickname, saveNickname } from './utils/history'
...
export default function App() {
  ...
  const restart = () => upd({
    step:1, ctx:{intent:'',alt:'',edge:''}, questions:[], answers:['','',''],
    skipped:[false,false,false], report:'', radar:null, error:'',
    feedback:[null,null,null], feedbackText:['','',''],
  })

  const genQuestions = () => {
    if (S.code.trim().length < 20) { upd({ error:'코드를 20자 이상 입력해주세요.' }); return }
    upd({ loading:true, error:'' })
    setTimeout(() => upd({ loading:false, questions:MOCK_QUESTIONS, step:3 }), 1800)
  }

  const genReport = () => {
    const valid = S.answers.filter((a,i) => !S.skipped[i] && a.trim().length >= 10)
    if (!valid.length) { upd({ error:'최소 1개 질문에 10자 이상 답변을 입력해주세요.' }); return }
    upd({ loading:true, error:'' })
    setTimeout(() => {
  ...
  const requestFeedback = (i) => {
    const fb = [...S.feedback]; fb[i] = 'loading'
    upd({ feedback: fb })
    setTimeout(() => {
      const fb2 = [...S.feedback], ft = [...S.feedbackText]
      fb2[i] = 'shown'; ft[i] = MOCK_FEEDBACK[i] || ''
      upd({ feedback: fb2, feedbackText: ft })
    }, 1300)
  }

  /* Auth views */
  if (S.view === 'login' || S.view === 'register') {
    return (
      <AuthPage
        view={S.view}
        onLogin={login}
    ...
    switch (S.step) {
      ...
      case 3: return (
        <Step3QA
          ...
          error={S.error}
          onAnswerChange={(i, val) => { const a=[...S.answers]; a[i]=val; upd({ answers:a, error:'' }) }}
          onSkip={(i) => { const sk=[...S.skipped]; sk[i]=true; upd({ skipped:sk }) }}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm text-t2 mb-1">
        {isReport ? '트러블슈팅 리포트를 작성하는 중...' : 'AI 면접 질문을 생성하는 중...'}
      </p>
      <p className="text-xs text-t3">
        {isReport ? '포트폴리오에 활용 가능한 리포트를 생성합니다.' : '기술 의사결정, 트러블슈팅 영역을 분석합니다.'}
      </p>
    </div>
  )
}
