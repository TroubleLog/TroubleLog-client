  ...
  return (
    <div className="animate-fade-up">
      <div className="mb-8">
        ...
        <p className="sec-sub">코드만으로 파악하기 어려운 의사결정 배경을 수집합니다. 모두 선택 사항이며, 입력할수록 질문의 깊이가 달라집니다.</p>
      </div>

      <ErrBar msg={error} />

      {CTX_META.map((q, i) => (
        <div key={q.id} className="bg-s1 border border-white/5 rounded-lg px-6 py-5 mb-3 focus-within:border-white/[0.16] transition-colors">
          <div className="flex items-center gap-2.5 mb-2.5">
            ...
            <span className="text-[10px] text-t3 ml-auto bg-s3 rounded px-1.5 py-0.5">선택</span>
          </div>
          <textarea
      ...
      <div className="flex gap-2.5 mt-6">
        ...
        <button className="btn-primary flex-1" onClick={onNext}>AI 면접 질문 생성 →</button>
      </div>
    </div>
  )
}
