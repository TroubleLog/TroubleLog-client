...
function QACard({ q, i, answer, skipped, feedback, feedbackText, onAnswerChange, onSkip, onFeedback }) {
  const badgeKey = BADGE_TYPE[q.type] || 'tech'
  const cnt = answer.length
  const animClass = i === 0 ? 'animate-fade-up' : i === 1 ? 'animate-fade-up2' : 'animate-fade-up3'

  return (
    <div className={`bg-s1 border border-white/5 rounded-lg overflow-hidden mb-3 ${animClass} ${skipped ? 'opacity-40' : ''} transition-colors focus-within:border-white/[0.16]`}>
      {/* Question */}
      <div className="px-6 py-5 border-b border-white/5">
        <div className="flex items-center gap-2 mb-2.5">
          <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full border ${BADGE_STYLE[badgeKey]}`}>
            {q.type}
          </span>
          <span className="text-[11px] text-t3 font-mono ml-auto">Q{i + 1}</span>
        </div>
        <p className="text-[14px] text-t1 leading-relaxed mb-3">{q.question}</p>
        <div className="flex flex-wrap gap-1.5">
          {q.hint.split(',').map((h, hi) => (
            <span key={hi} className="text-[11px] text-t3 bg-s2 border border-white/5 rounded px-2 py-0.5 font-mono">
              {h.trim()}
            </span>
          ))}
        </div>
      ...
      <div className="px-6 py-[18px] bg-white/[0.018]">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-semibold text-t3 tracking-wider uppercase">내 답변</span>
          {skipped
            ? <span className="text-[10px] text-t3 bg-s3 border border-white/5 rounded px-2 py-0.5">건너뜀</span>
            : <button className="text-[11px] text-t3 underline underline-offset-2 bg-transparent border-none cursor-pointer hover:text-t2 transition-colors" onClick={() => onSkip(i)}>건너뛰기</button>
          }
        </div>
        ...
        {!skipped && (
          <>
            ...
            />
            <p className={`text-[10px] font-mono mt-1.5 ${cnt > 0 && cnt < 10 ? 'text-red-400' : 'text-t3'}`}>
            ...
            {feedback === 'shown' && (
              <div className="bg-[var(--gd)] border border-accent/15 rounded-md px-3.5 py-3 mt-2.5 animate-fade-up">
                <p className="text-[10px] font-semibold text-accent tracking-wide mb-1.5">✦ AI 피드백</p>
                <p className="text-[12px] text-t2 leading-relaxed">{feedbackText}</p>
              </div>
            )}
            {feedback === null && (
              <button
...
export default function Step3QA({ questions, answers, skipped, feedback, feedbackText, error, onAnswerChange, onSkip, onFeedback, onBack, onNext }) {
  return (
    <div className="animate-fade-up">
      <div className="mb-8">
        ...
        <p className="sec-sub">코드와 컨텍스트를 분석하여 생성된 질문입니다. 각 질문 아래에 자유롭게 답변을 작성하세요.</p>
      </div>
      ...
      <div className="flex gap-2.5 mt-6">
        ...
        <button className="btn-primary flex-1" onClick={onNext}>트러블슈팅 리포트 생성 →</button>
      </div>
    </div>
  )
}
