...
const CheckIcon = () => (
  <svg className="w-2.5 h-2.5 stroke-white fill-none stroke-[2.5] [stroke-linecap:round] [stroke-linejoin:round]" viewBox="0 0 10 10">
  ...
  return (
    <aside className="w-[250px] flex-shrink-0 border-r border-white/5 bg-s1 flex flex-col sticky top-0 h-screen">
      ...
      <nav className="flex-1 px-3 py-5 overflow-y-auto">
        <p className="text-[10px] font-semibold text-t3 tracking-widest uppercase px-2 mb-2">진행 단계</p>
        {STEPS_META.map((s, i) => {
          const n = i + 1
          const done = n < step
          const active = n === step
          return (
            <div key={i}>
              <div className={`flex items-start gap-2.5 px-2.5 py-2 rounded-md mb-0.5 transition-colors ${active ? 'bg-s2' : ''}`}>
                <div className={`
                  ...
                  : active ? 'bg-accent text-white shadow-[0_0_0_4px_var(--gd2)]'
                           : 'bg-s3 border border-white/[0.09] text-t3'}
                `}>
                  {done ? <CheckIcon /> : n}
                </div>
                <div>
                  <div className={`text-[13px] font-medium leading-snug transition-colors ${active ? 'text-t1' : 'text-t2'}`}>{s.name}</div>
                  <div className="text-[11px] text-t3 mt-0.5">{s.desc}</div>
                </div>
              </div>
              {i < 3 && (
                <div className={`w-px h-3.5 ml-[21px] mb-0.5 transition-colors ${done ? 'bg-accent/40' : 'bg-white/5'}`} />
              )}
            </div>
          )
        })}
      ...
      <div className="px-5 py-3 border-t border-white/5">
        <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12px] font-medium text-t1 truncate">{user?.nickname}</div>
